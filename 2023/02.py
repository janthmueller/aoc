...
class Hand(dict):
    def __init__(self, s):
        hand_dict = {"green": 0, "red": 0, "blue": 0}
        for cube_repr in s.split(","):
            cube_list = cube_repr.split(" ")
            hand_dict[cube_list[-1]] = int(cube_list[1])
        super().__init__(hand_dict)


class Game:
    def __init__(self, s):
        s_id, s_hands = s.split(":")
        self.id = int(s_id[5:])
        self.hands = [Hand(s) for s in s_hands.split(";")]
        self._max_grabbed = {}

    @property
    def max_grabbed(self):
        if not self._max_grabbed:
            self._max_grabbed = {"green": 0, "red": 0, "blue": 0}
            for hand in self.hands:
                for k, v in self._max_grabbed.items():
                    self._max_grabbed[k] = max(v, hand[k])
        return self._max_grabbed


class Bag:
    def __init__(self, n_red, n_green, n_blue):
        self.red = n_red
        self.green = n_green
        self.blue = n_blue

    def check_game(self, game):
        if self.red < game.max_grabbed["red"]:
            return False
        elif self.green < game.max_grabbed["green"]:
            return False
        elif self.blue < game.max_grabbed["blue"]:
            return False
        return True
...
bag = Bag(12, 13, 14)
id_sum = 0
for s_game in input_as_list:
    game = Game(s_game)
    if bag.check_game(game):
        id_sum += game.id
print(id_sum)
...
for s_game in input_as_list:
    game = Game(s_game)
    power = 1
    for v in game.max_grabbed.values():
        power *= v
    power_sum += power
print(power_sum)
