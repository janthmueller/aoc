...
def decode_str(s):
    ...
    last_occurence = None
    for i, char_from_start in enumerate(s):
        char_from_end = s[length_of_str-i-1]
        if not first_occurence:
            first_occurence = num_or_none(char_from_start)
        elif last_occurence:
            break
        if not last_occurence:
            last_occurence = num_or_none(char_from_end)
        elif first_occurence:
            break
    return first_occurence * 10 + last_occurence

result = 0
for s in input_as_list:
    result += decode_str(s)
print(result)

# part 2

spelled_letter = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight":8, "nine": 9}
...
def decode_str(s):
    ...
    last_occurence = None
    for i, char_from_start in enumerate(s):
        index_from_end = length_of_str-i-1
        char_from_end = s[index_from_end]
        if not first_occurence:
            first_occurence = num_or_none(char_from_start)
            if not first_occurence:
                for j in range(3,6):
                    first_occurence = spelled_letter.get(s[i:min(length_of_str-1, i+j)], None)
                    if first_occurence:
                        break
        elif last_occurence:
            break
        if not last_occurence:
            ...
            if not last_occurence:
                for j in range(3,6):
                    last_occurence = spelled_letter.get(s[max(index_from_end-j + 1,0): index_from_end + 1], None)
                    if last_occurence:
                        break
        elif first_occurence:
            break
